...
# we may make the moves "zzxy" -> "xzzy" -> "xyzz" that swap S[0] and S[2], then S[1] and S[3].
#
...
class Solution:
    def numSpecialEquivGroups(self, A: List[str]) -> int:
        return len(
            {(tuple(sorted(i[::2])), tuple(sorted(i[1::2]))) for i in A})
...
Test(Solution().numSpecialEquivGroups, cases, True).test()
